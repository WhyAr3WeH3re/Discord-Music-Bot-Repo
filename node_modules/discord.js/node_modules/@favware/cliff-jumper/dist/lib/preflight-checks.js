...
export async function preflightChecks(options) {
    ...
    const packageJsonExistsInCwd = await doActionAndLog('Checking if package.json exists in the current working directory', fileExists(packageJsonPath));
    ...
    const packageJsonContent = await doActionAndLog('Checking if package.json has a version property', //
    ...
    if (!options.skipTag) {
        const hasCliffConfigAtCwd = await doActionAndLog('Checking if a cliff.toml exists in the current working directory', //
        ...
        const hasChangelogFileAtCwd = await doActionAndLog('Checking if a CHANGELOG.md exists in the current working directory', //
        fileExists(join(packageCwd, 'CHANGELOG.md')));
        if (!hasChangelogFileAtCwd) {
            if (options.firstRelease) {
                await doActionAndLog('Creating an empty CHANGELOG.md file in the current working directory', //
        ...
        const hasGitCliff = await doActionAndLog('Checking if git cliff is installed', //
        gitCliffExists());
        if (!hasGitCliff) {
            logVerboseError({
